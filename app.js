...
function renderStatTab(c, tab, stats) {
  ...
  if (tab === 'ranking') {
    ...
    c.innerHTML = `<p class="section-title">🏆 Ranking — Victorias</p><div class="card">${stats.ranking.map((r, i) => `<div class="stat-row"><div class="stat-pos ${i === 0 ? 'gold' : i === 1 ? 'silver' : i === 2 ? 'bronze' : ''}">${i === 0 ? '🥇' : i === 1 ? '🥈' : i === 2 ? '🥉' : i + 1}</div><div class="stat-avatar" style="background:${r.color}">${initials(r.name)}</div><div style="flex:1;min-width:0"><div class="stat-name">${escHtml(r.name)}</div><div class="stat-bar-wrap" style="max-width:120px"><div class="stat-bar" style="width:${Math.round((r.wins / max) * 100)}%;background:linear-gradient(90deg,var(--accent),var(--cyan))"></div></div></div><div style="text-align:right"><div class="stat-val">${r.wins} <span style="font-size:0.6em;color:var(--text-muted)">/ ${r.matches}</span></div><div class="stat-unit">Ratio: ${r.matches > 0 ? Math.round((r.wins / r.matches) * 100) : 0}%</div></div></div>`).join('')}</div>`;
  } else if (tab === 'games') {
